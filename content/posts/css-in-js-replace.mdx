...
React 생태계에서 styled-components와 emotion으로 대표되는 런타임 CSS-in-JS는 오랫동안 표준처럼 사용되어왔다. 하지만 Next.js App Router(React Server Components) 도입 이후 성능 최적화 기준이 높아지면서, 런타임 오버헤드를 발생시키는 기존 방식의 한계가 드러난다.

대안으로 Tailwind CSS가 부상하고 있지만, HTML 클래스 속성이 길어지는 단점을 피하면서 JS/TS 문법으로 스타일을 캡슐화하려는 요구는 계속된다. 이러한 간극을 메우기 위해 등장한 기술이 **Zero-Runtime CSS-in-JS**다.
...
## React Server Components(RSC) 아키텍처와의 완벽한 호환성
