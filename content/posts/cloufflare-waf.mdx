...
GhostGains 프로젝트의 백엔드 API(`api.ghostgains.app`)는 서비스 구조상 퍼블릭하게 열려있었다. cloudflare workers로 서버리스 api가 d1 데이터베이스와 연결되어있는데 아무런 보호 조치 없이 열어두자니 크롤링이나 악성 트래픽이 걱정됐다. 실제로 구글애널틱스로 프론트 상황만 봐도 각종 나라에서 마구잡이로 요청이 들어온다.
이건 누군가 스크립트로 요청을 난사하면 **D1(DB) 읽기 비용**이나 **Workers 호출 비용**이 줄줄 샐 수 있다.
